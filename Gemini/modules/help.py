...
async def help(event):
    if event.is_group:
       await event.reply(HELP_TEXT.format(bot_username=bot.get_me().username))
       return

    await event.respond(HELP_TEXT.format(bot_username=bot.get_me().username), buttons=[[Button.inline("Kembali", data="home")]])


@bot.on(events.callbackquery.CallbackQuery(data="help"))
async def chelp(event):
     await event.edit(HELP_TEXT.format(bot_username=bot.get_me().username), 
     buttons=[
